...
> non-blocking assignments (`<=`) on the same signals (e.g. `z_m`, `guard_bit`,
> `round_bit`, `sticky`), each block will read the *pre-cycle* register value,
> not the updated value from the block above it — this silently produces wrong
> results whenever more than one of these sub-steps is needed for the same
> operand.
>
